...
class Task:
    def __init__(self,batch_size,device,block_size):
        ...
        dataset = load_dataset("cognitivecomputations/dolphin", data_files="flan1m-alpaca-uncensored.jsonl", split='train', streaming=False,num_proc=28)
        ...
        tokenized_datasets = train_dataset.map(self.tokenize_function, batched=True, remove_columns={'instruction','input','output'})
        tokenized_val_datasets = val_dataset.map(self.tokenize_function, batched=True, remove_columns={'instruction','input','output'})
        
        # tokenized_datasets = tokenized_datasets.with_format("torch")
        # tokenized_val_datasets=tokenized_val_datasets.with_format("torch")
        self.train_loader = DataLoader(tokenized_datasets, batch_size=batch_size,  pin_memory=True)
        self.val_loader = DataLoader(tokenized_val_datasets, batch_size=batch_size, pin_memory=True)
    def tokenize_function(self,examples):
        # Tokenize the text
        combined_texts = [instr + " " + inp + " " + outp 
                      for instr, inp, outp in zip(examples['instruction'], examples['input'], examples['output'])]
        tokenized_output = self.tokenizer(combined_texts, padding='max_length', truncation=True, max_length=self.block_size, return_tensors='pt')
